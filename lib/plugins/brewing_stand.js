...
function inject (bot) {
  ...
  /**
   * @param {import('prismarine-block').Block} brewingStandBlock
   */
  async function openBrewingStand (brewingStandBlock) {
    ...
    /** @type {import('../..').BrewingStand} */ // @ts-ignore
    ...
    async function takePotions () {
      ...
      for (const i of [0, 1, 2]) {
        if (brewingStand.potions()[i]) {
          res.push(await takeSomething(brewingStand.potions()[i]))
        }
      }
      return res
    }
